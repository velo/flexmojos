...
public class SourceViewMojo extends AbstractIrvinMojo
{
    ...
    protected void setUp() throws MojoExecutionException, MojoFailureException
    {
        ...
        if (velocityEngine == null)
        {
            ...
            try
            {
                ...
                p.setProperty("classpath." + VelocityEngine.RESOURCE_LOADER + ".class", ClasspathResourceLoader.class.getName());
     ...
     * @param targetDirectory
     *            The directory where to store the output file.
     * @throws MojoFailureException
     *             If the template could not be loaded, parsed, or the output
     *             file could not be written.
     */
    protected void processTemplate(String templateName, File targetDirectory) throws MojoFailureException
    {
        FileWriterWithEncoding pageWriter = null;
        try
        {
            pageWriter = new FileWriterWithEncoding(new File(targetDirectory.getCanonicalPath() + File.separator + templateName),
    ...
    {
        File mainSourceFile = SourceFileResolver.resolveSourceFile(project.getCompileSourceRoots(),
                                                                   null,
                                                                   project.getGroupId(),
