...
public class IT0015CoverageTest extends AbstractMavenIntegrationTestCase {
	...
	public void testHtmlwrapperTemplates() throws Exception {
		File testDir = ResourceExtractor.simpleExtractResources(
				MavenVerifierHelper.class, "/coverage/htmlwrapper-templates");
		customTester(testDir, "package");
		File folder = new File(testDir,
				"folder/target/htmlwrapper-templates-folder-1.0-SNAPSHOT.html");
		assertTrue("Should generate htmlwrapper" + folder.getAbsolutePath(),
				folder.exists());
		File zip = new File(testDir,
				"zip/target/htmlwrapper-templates-zip-1.0-SNAPSHOT.html");
		assertTrue("Should generate htmlwrapper " + zip.getAbsolutePath(), zip
				.exists());
	}
	...
	public void testCompilationOptions() throws Exception {
